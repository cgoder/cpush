...
int deliverMessage(Client* c, MQTTString* topicName, MQTTMessage* message)
{
    ...
    if (rc == FAILURE && c->defaultMessageHandler != NULL) 
    {
        // MessageData md;
        // NewMessageData(&md, topicName, message);
        c->defaultMessageHandler(message);
        rc = SUCCESS;
    }   
...
/*@return error code. <0 is failure, >=0 is success.by real mqtt protocol return code.*/
int MQTTConnect(Client* c, MQTTPacket_connectData* options)
{
    ...
    {
        unsigned char connack_rc = 255;/*connack return code*/
...
// int MQTTSubscribeAll(Client* c, const char* topicFilter[], char topicCount, enum QoS qos[], messageHandler messageHandler[])
// { 
//     int rc = FAILURE;  
//     Timer timer;
//     int len = 0;
//     MQTTString* topics = (MQTTString*)calloc(1,sizeof(MQTTString)*topicCount);
//     if (!topics) {
//         goto exit;
//     }
//     for (int i=0;i<topicCount;++i) {
//         topics[i]->cstring = (char *)topicFilter[i];
//     }
...
//     len = MQTTSerialize_subscribe(c->buf, c->buf_size, 0, getNextPacketId(c), topicCount, topics, (int*)qos);
//     if (len <= 0)
//         goto exit;
...
//         unsigned short mypacketid;
//         if (MQTTDeserialize_suback(&mypacketid, topicCount, &count, grantedQoS, c->readbuf, c->readbuf_size) == 1)
//         {
//             rc = SUCCESS;
//             for (int i=0;i<count;++i) 
//             {
//                 if ((grantedQoS[i] != 0x80)||(grantedQoS[i] != 0x01)||(grantedQoS[i] != 0x02)||(grantedQoS[i] != 0x00))
//                 {
//                     continue;
//                 }
//                 if (grantedQoS[i] != 0x80)
//                 {
//                     // if (c->messageHandlers[j].topicFilter == 0)
//                     {
//                         c->messageHandlers[i].topicFilter = topicFilter[i];
//                         c->messageHandlers[i].fp = messageHandler[i];
//                         rc = 0;
//                         break;
//                     }                  
//                 }
